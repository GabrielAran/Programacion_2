...
public class QueueWithPriorityGenerico<T> {
    private PriorityNodeGenerico<T> first;

    public void add(T a, int priority) {
        // 0 elementos
        if (this.first == null) {
            this.first = new PriorityNodeGenerico<T>(a, priority, null);
            return;
        }
        // 0 elementos
        ...
        if (this.first.getNext() == null) {
            if (this.first.getPriority() > priority) { // si la prio del primero es > nuevo, entonces lo pone adelante
                this.first = new PriorityNodeGenerico<T>(a, priority, this.first);
            } else { // si la prio del primero es < nuevo, entonces le pone como next al primero, el nuevo
                this.first.setNext(new PriorityNodeGenerico<T>(a, priority, null));
            }
            return;
        }
        // 1 elemento -->
        ...
        PriorityNodeGenerico<T> primero = this.first; // primero
        PriorityNodeGenerico<T> segundo = this.first.getNext(); // segundo
        while (segundo.getNext() != null) { // si el segundo tiene next
            if (segundo.getPriority() > priority) {
                primero.setNext(new PriorityNodeGenerico<T>(a, priority, segundo));
                return;
            }
        ...
        if (segundo.getPriority() > priority) {
            ...
            segundo.setNext(new PriorityNodeGenerico<T>(a, priority, null));
        }
    }
