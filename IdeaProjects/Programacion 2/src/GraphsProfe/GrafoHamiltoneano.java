...
public class GrafoHamiltoneano implements IGraph {
    private static final int MAX_NODES = 15;
    private final int[][] adjacencyMatrix;
    private final IDictionary dictionary;
    private int contValues;

    public GrafoHamiltoneano() {
    ...
    public boolean isPossiblyAnOrderedList(){
        int i = 0;
        int j = 1;
        while (j < this.contValues){ // chequeo lo que si tiene que pasar
            if (adjacencyMatrix[i][j] == 0){
                System.out.println("meli " + i + " melj " + j);
                return false;
            }
            i++;
            j++;
        }
        //chequear lo q no deberia pasar:
        for (int q = 0; q < adjacencyMatrix.length; q++){
            for (int w = 0; w < adjacencyMatrix.length; w++){
                if ((w == 0 || q == this.contValues - 1) && adjacencyMatrix[q][w] != 0){ // la primera columna y ultima fila tienen nada
                    return false;
                }
                if (q == w && adjacencyMatrix[q][w] != 0){ // no se puede autodirigir
                    return false;
                }
                if ((q - w > 1 || q - w < -1) && adjacencyMatrix[q][w] != 0){ // no puede haber mas d 2 d diferencia
                    return false;
                }
                if (q > w && adjacencyMatrix[q][w] != 0){ // nunca la fila es mayor a la columna
                    return false;
                }
            }
        }
